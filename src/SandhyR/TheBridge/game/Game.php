...
class Game
{
        ...
        switch ($this->phase) {
            case "LOBBY":
                foreach ($this->players as $player) {
                    if ($player->isOnline()) {
                        ScoreFactory::setObjective($player, TextFormat::YELLOW . TextFormat::BOLD . "§l§aThe§2Bridge    ");
                        ScoreFactory::setScoreLine($player, 1, "           ");
                        ScoreFactory::setScoreLine($player, 2, "§5Blossom §l§d»§r TheBridge");
                        ScoreFactory::setScoreLine($player, 3, "      ");
                        ScoreFactory::setScoreLine($player, 4, TextFormat::YELLOW . "Players: " . TextFormat::WHITE . count($this->players) . "/2");
                        ScoreFactory::setScoreLine($player, 5, TextFormat::YELLOW . "Map: " . TextFormat::WHITE . $this->arenainfo["arenaname"]);
            ...
            case "COUNTDOWN":
                foreach ($this->players as $player) {
                    if ($player->isOnline()) {
                        ...
                        ScoreFactory::setScoreLine($player, 6, "§eMode: " . TextFormat::WHITE . "Solos");
                        ScoreFactory::setScoreLine($player, 7, "    ");
                        ScoreFactory::setScoreLine($player, 8, "§aStarting in: " . TextFormat::WHITE . $this->countdown . "s");
                        ScoreFactory::setScoreLine($player, 9, "     ");
                        ScoreFactory::setScoreLine($player, 10, TheBridge::getInstance()->getConfig()->get("server-ip"));
                        ScoreFactory::sendObjective($player);
                        ScoreFactory::sendLines($player);
            ...
            case "RUNNING":
                foreach ($this->players as $player) {
                    if ($player->isOnline()) {
                        ...
                        ScoreFactory::setScoreLine($player, 2, "§5Blossom §l§d»§r TheBridge");
                        ScoreFactory::setScoreLine($player, 3, "     ");
                        ScoreFactory::setScoreLine($player, 4, TextFormat::YELLOW . "Time Remaining: " . TextFormat::WHITE . Utils::intToString($this->timer));
                        ScoreFactory::setScoreLine($player, 5, TextFormat::YELLOW . "Map: §r" . $this->arenainfo["arenaname"]);
                        ScoreFactory::setScoreLine($player, 6, TextFormat::YELLOW . "Mode: §rSolos");
                        ScoreFactory::setScoreLine($player, 7, " ");
                        ScoreFactory::setScoreLine($player, 8, TextFormat::RED . TextFormat::BOLD . "[R]" . TextFormat::RESET . Utils::RintToPoint($this->playerinfo[array_search("red", $this->teams)]["goals"] ?? 0));
                        ScoreFactory::setScoreLine($player, 9, TextFormat::BLUE . TextFormat::BOLD . "[B]" . TextFormat::RESET . Utils::BintToPoint($this->playerinfo[array_search("blue", $this->teams)]["goals"] ?? 0));
                        ScoreFactory::setScoreLine($player, 10, "   ");
                        ScoreFactory::setScoreLine($player, 11, TextFormat::AQUA . "Kills: " . TextFormat::WHITE . $this->playerinfo[strtolower($player->getName())]["kills"]);
                        ScoreFactory::setScoreLine($player, 12, TextFormat::AQUA . "Goals: " . TextFormat::WHITE . $this->playerinfo[strtolower($player->getName())]["goals"]);
                        ScoreFactory::setScoreLine($player, 13, "     ");
                        ScoreFactory::setScoreLine($player, 14, TheBridge::getInstance()->getConfig()->get("server-ip"));
                        ScoreFactory::sendObjective($player);
                        ScoreFactory::sendLines($player);
                    }
                }
            ...
            case "RESTARTING":
                foreach ($this->players as $player) {
                    if ($player->isOnline()) {
                        ...
                        ScoreFactory::setScoreLine($player, 2, "§5Blossom §l§d»§r TheBridge");
                        ScoreFactory::setScoreLine($player, 3, "     ");
                        ScoreFactory::setScoreLine($player, 4, TextFormat::WHITE . "§eRestarting in: " . TextFormat::WHITE . $this->restartcountdown);
                        ScoreFactory::setScoreLine($player, 5, TextFormat::WHITE . "§eMap: §r" . $this->arenainfo["arenaname"]);
                        ScoreFactory::setScoreLine($player, 6, TextFormat::WHITE . "§eMode: §rSolos");
                        ScoreFactory::setScoreLine($player, 7, " ");
                        ScoreFactory::setScoreLine($player, 8, TextFormat::RED . TextFormat::BOLD . "[R]" . TextFormat::RESET . Utils::RintToPoint($this->playerinfo[array_search("red", $this->teams)]["goals"] ?? 0));
                        ScoreFactory::setScoreLine($player, 9, TextFormat::BLUE . TextFormat::BOLD . "[B]" . TextFormat::RESET . Utils::BintToPoint($this->playerinfo[array_search("blue", $this->teams)]["goals"] ?? 0));
                        ScoreFactory::setScoreLine($player, 10, "   ");
                ...
                if ($this->restartcountdown <= 0) {
                    ...
                    if ($game <= 0) {
                        foreach ($this->players as $player) {
                            ScoreFactory::removeObjective($player);
                            $player->sendMessage(TextFormat::RED . "§l§d»§r§c No arenas are currently running!");
    ...
    public function broadcastMessage(Player $player, string $message)
    {
        foreach ($this->players as $p) {
            $p->sendMessage($this->getTeamChatFormat($player) . " " . TextFormat::WHITE . $player->getName() . " §l§8»§r " . $message);
        }
    }
    ...
    private function getTeamChatFormat(Player $player): string
    {
        if ($this->teams[strtolower($player->getName())] == "blue") {
            return TextFormat::BLUE . "§8[§9BLUE§8]";
        }
        return TextFormat::RED . "§8[§cRED§8]";
    }
    ...
    public function addGoal(Player $player): bool
    {
        if ($this->playerinfo[strtolower($player->getName())]["goals"] <= 5) {
            ...
            $this->broadcastCustomMessage(Utils::teamToColor($this->teams[strtolower($player->getName())]) . $player->getName() . TextFormat::YELLOW . " scored§g! §7§oSwish!");
            if ($this->playerinfo[strtolower($player->getName())]["goals"] >= 5) {
    ...
    public function handleDeath(Player $player, EntityDamageEvent $event): void
    {
        if ($event instanceof EntityDamageByEntityEvent) {
            ...
            if ($damager instanceof Player) {
                $this->broadcastCustomMessage(Utils::teamToColor($this->teams[strtolower($player->getName())]) . $player->getName() . TextFormat::YELLOW . " was killed by " . Utils::teamToColor($this->teams[strtolower($damager->getName())]) . $damager->getName() . "§g!");
                $this->playerinfo[strtolower($player->getName())]["damager"] = null;
                ++$this->playerinfo[strtolower($damager->getName())]["kills"];
            } else {
                $this->broadcastCustomMessage(Utils::teamToColor($this->teams[strtolower($player->getName())]) . $player->getName() . TextFormat::YELLOW . " perished in the void§g!");
            }
        }
